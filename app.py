...
.rot-tag{font-size:10px;font-family:'IBM Plex Mono',monospace;background:#0a1a2a;color:#3a9bd5;border:1px solid #1a4a7a;border-radius:2px;padding:2px 6px;margin-left:6px}
...
.rot-box{background:#0a1020;border:1px solid #1a3a5a;border-left:3px solid #3a7bd5;border-radius:4px;padding:10px 14px;margin:6px 0;font-size:12px;font-family:'IBM Plex Mono',monospace}
...
def consenso_claude_cuerpo(client, img: Image.Image, prompt: str, modelo: str, intentos: int = 1) -> dict:
    """
    Para el cuerpo (sin YOLO): múltiples intentos, votación por clase.
    """
    img_proc = preproc(img)
    b64 = pil_b64(img_proc)
    res = []
    ...
    if not res:
        return {"clase": "ERROR", "confianza": 0.0, "descripcion": "Sin respuesta", "fuente": "Claude"}

    ganador = Counter(r.get("clase", "ERROR") for r in res).most_common(1)[0][0]
    confs = [r.get("confianza", 0.0) for r in res if r.get("clase") == ganador]
    desc = next((r.get("descripcion", "") for r in res if r.get("clase") == ganador), "")
    return {
        "clase": ganador,
...
def retroalimentar_claude(client, img, res_yolo, modelo):
    img = preproc(img)
    b64 = pil_b64(img)
    prompt = PROMPT_CUERPO_RETRO.format(
        clase_yolo=res_yolo["clase"],
        conf_yolo=res_yolo["confianza"]
    )
    try:
...
def es_nc(rc, re, modo_estricto=False):
    ...
    if estado_e in ("VENCIDA", "SIN_FECHA"):
        if cat_e == "confirmada":
            motivos.append(f"Etiqueta {estado_e.lower()}")
        elif cat_e == "dudosa":
            motivos.append(f"Etiqueta {estado_e.lower()} (DUDOSA)")
    elif estado_e == "ILEGIBLE" and cat_e == "confirmada":
        motivos.append("Etiqueta ilegible (confirmado)")

    return (
        len(motivos) > 0,
        " · ".join(motivos) if motivos else "Conforme",
        cat_e
    )
...
def reporte_pdf(resultados, N, n, c, decision, X, sid, correcciones_count, config=None):
    ...
    buf = io.BytesIO()
    cv = rlcanvas.Canvas(buf, pagesize=A4)
    W, H = A4
    now = datetime.now()
    ...
    for idx, lata in enumerate(resultados):
        ...
        cv.setFillColor(colors.HexColor("#3a7bd5")); cv.drawString(cx[6]+3, y-4, votos_str)
        cv.setFillColor(dec_color); cv.setFont("Helvetica-Bold", 7)
        cv.drawString(cx[7]+3, y-4, dec_txt)
        y -= 18
...
with st.sidebar:
    ...
    retro_activo = st.toggle("Activar retroalimentación", value=False,
        help="Claude Vision revisa cada decisión de YOLO sobre el cuerpo y la corrige si difiere.")
    ...
    if retro_activo:
        st.markdown(
            ...
            '<span style="color:#6a5a8a">YOLO analiza → Claude revisa → si difiere, Claude gana</span>'
    ...
    if YOLO_DISPONIBLE:
        ...
        if ym:
            ...
            st.markdown('<div style="background:#1a1a07;border:1px solid #5a5a1a;border-left:3px solid #f0b429;border-radius:4px;padding:8px 12px;font-family:monospace;font-size:12px"><span style="color:#f0b429">⚠ YOLO: modelo NO ENCONTRADO</span></div>', unsafe_allow_html=True)
    else:
...
if not api_key:
    st.warning("Ingresa tu API Key en el panel izquierdo.", icon="🔑")
else:
...
with tab_plan:
    ...
    if modo_plan.startswith("🤖"):
        ...
        with col2:
            st.markdown(
                f'<div class="plan-box"><div class="plan-row">'
                f'<span><span class="plan-param">LETRA: </span><span class="plan-val">{letra}</span></span>'
                f'<span><span class="plan-param">n = </span><span class="plan-val">{n_auto}</span></span>'
                f'<span><span class="plan-param">ACEPTAR X ≤ </span><span class="plan-val">{c_auto}</span></span>'
                f'<span><span class="plan-param">RECHAZAR X > </span><span class="plan-val">{c_auto}</span></span>'
                f'</div></div>', unsafe_allow_html=True)
        n_plan, c_plan = n_auto, c_auto
    else:
        st.info("En modo manual defines N, n y c libremente.", icon="✏️")
        col_m1, col_m2, col_m3, col_m4 = st.columns(4)
        with col_m1:
            N_input = st.number_input("N – Tamaño del lote", min_value=2, max_value=500000, value=60, step=1, key="N_man")
        letra_ref, n_ref, c_ref = iso_plan(N_input)
        with col_m2:
            n_plan = st.number_input(f"n – Muestra (ISO sugiere {n_ref})", min_value=1, max_value=N_input, value=n_ref, step=1)
        with col_m3:
            c_plan = st.number_input(f"c – Aceptación (ISO sugiere {c_ref})", min_value=0, max_value=n_plan, value=c_ref, step=1)
        with col_m4:
            st.metric("Letra ISO referencia", letra_ref)
        if n_plan != n_ref or c_plan != c_ref:
            st.warning(f"⚠️ Usando n={n_plan}, c={c_plan} en lugar de n={n_ref}, c={c_ref} recomendados.", icon="⚠️")

    alpha_p, nql_p = riesgos(n_plan, c_plan)
    pa_nca_p = pa_bin(0.025, n_plan, c_plan)
    st.markdown(
        f'<div class="metrics-row">'
        f'<div class="met neu"><span class="met-val met-neu">{N_input}</span><div class="met-lbl">Tamaño lote N</div></div>'
        f'<div class="met neu"><span class="met-val met-neu">{n_plan}</span><div class="met-lbl">Muestra n</div></div>'
        f'<div class="met warn"><span class="met-val met-warn">{c_plan}</span><div class="met-lbl">Núm. aceptación c</div></div>'
        f'<div class="met ok"><span class="met-val met-ok">{pa_nca_p:.0%}</span><div class="met-lbl">Pa en NCA (1−α)</div></div>'
        f'</div>', unsafe_allow_html=True)
    st.info(
        f"**Interpretación:** Inspecciona **{n_plan}** latas. "
        f"Si X ≤ {c_plan} → ACEPTAS. Si X ≥ {c_plan+1} → RECHAZAS. "
        f"Pa en NCA = {pa_nca_p:.1%} (α = {alpha_p:.1%}).", icon="📐")
    ...
    with col_r2:
        ...
        if _fm_activa:
            st.markdown(
                ...
                f'📅 Fecha mínima activa: <b style="color:#f0b429">{_fm_activa}</b></div>',
                unsafe_allow_html=True)
...
with tab_insp:
    ...
    with c1:
        N_lote = st.number_input("N – Tamaño del lote", min_value=2, max_value=500000,
                                  value=N_input, step=1, key="N_insp")
    ...
    if st.button("🔬 INICIAR INSPECCIÓN", disabled=not puede, type="primary"):
        ...
        for i in range(total):
            ...
            if nc:
                X += 1
            elif res_c.get("clase") == "MENOR" or cat_etiqueta in ("dudosa", "ilegible"):
                obs += 1
            if cat_etiqueta == "dudosa":
                dudosas_count += 1
            ...
            resultados.append({
                ...
                "img_cuerpo_bytes": bytes_c,
                "img_etiqueta_bytes": bytes_e,
                "corregido": corregido,
                "yolo_original": yolo_orig
            })
        ...
        retro_txt_dec = f"  Correcciones YOLO: {correcciones}" if retro_activo and correcciones > 0 else ""
        sub_txt = f"X = {X} {simbolo_dec} c = {c_lote} | p = {pp:.1%} | {total} latas{retro_txt_dec}"
        st.markdown(
            f'<div class="decision-box {cls_d}">'
            f'<div class="decision-title" style="color:{cv_}">{decision}</div>'
            f'<div class="decision-sub">{sub_txt}</div></div>',
            unsafe_allow_html=True)

        # ── Métricas ─────────────────────────────────────────────────────
        cl_x = "ok" if decision == "ACEPTADO" else "bad"
        cv_x = "met-ok" if decision == "ACEPTADO" else "met-bad"
        st.markdown(
            f'<div class="metrics-row">'
            f'<div class="met {cl_x}"><span class="met-val {cv_x}">{X}</span><div class="met-lbl">No conformes X</div></div>'
            f'<div class="met neu"><span class="met-val met-neu">{total-X-obs}</span><div class="met-lbl">Conformes</div></div>'
            f'<div class="met warn"><span class="met-val met-warn">{obs}</span><div class="met-lbl">Observaciones</div></div>'
            f'<div class="met neu"><span class="met-val met-neu">{pp:.1%}</span><div class="met-lbl">p\' estimada</div></div>'
            f'</div>', unsafe_allow_html=True)

        if retro_activo and correcciones > 0:
            st.markdown(
                f'<div class="retro-box">🔄 Retroalimentación: Claude corrigió a YOLO en '
...
with tab_hist:
    ...
    ps, pas = calcular_cco(n_c, c_c)
    alpha_v, nql_v = riesgos(n_c, c_c)
    ...
    try:
        ...
        if nql_v:
            ax.axvline(nql_v, color="#e55353", linestyle=":", linewidth=1, alpha=0.8,
                       label=f"NQL≈{nql_v:.0%}")
        ...
        ax.set_title(f"CCO · n={n_c}, c={c_c} · ISO 2859-1 · NCA=2.5%",
    ...
    st.markdown(
        f'<div class="metrics-row">'
        f'<div class="met warn"><span class="met-val met-warn">{alpha_v:.1%}</span><div class="met-lbl">α Riesgo productor</div></div>'
        f'<div class="met bad"><span class="met-val met-bad">≤10%</span><div class="met-lbl">β Riesgo consumidor</div></div>'
        f'<div class="met ok"><span class="met-val met-ok">{pa_bin(0.025,n_c,c_c):.0%}</span><div class="met-lbl">Pa en NCA</div></div>'
        f'<div class="met neu"><span class="met-val met-neu">{nql_s}</span><div class="met-lbl">NQL estimado</div></div>'
        f'</div>', unsafe_allow_html=True)
    ...
    if st.session_state.historial_lotes:
        ...
        st.markdown("#### Historial de lotes")
        st.dataframe([{
            "Lote": l["id"], "N": l["N"], "n": l["n"], "c": l["c"], "X": l["X"],
            "p'": f"{l['X']/l['n']:.1%}" if l["n"] > 0 else "—",
            "Decisión": l["decision"]
